...
A poe.ninja-style price-tracking website for sealed Pokemon TCG product — booster packs, booster boxes, booster bundles, and Elite Trainer Boxes — sourced from eBay. **v1.0 ships live current-asking-price tracking (with 7d/30d trend indicators)** for English-only product across the 4 most recent sets. Real sold-price history was the original dual-data vision but is deferred — the eBay Marketplace Insights API access request was denied (2026-08-02) — and rolls into a future milestone if/when that access path reopens (see Context, FALLBACK-DECISION.md).

## Core Value

A user can look up a specific pack/box/ETB and see whether it's priced fairly right now, backed by live eBay asking prices — this must always be accurate and current. (Original v1 framing also promised sold-price history as a second pillar; that pillar is deferred to a future milestone, not dropped — see Context.)
...
### Deferred (formerly Active, blocked on external access)

- [ ] Sold-listing ingestion via the Marketplace Insights API — the eBay Application Growth Check (ticket 260802-000004) was **denied 2026-08-02**. Per `FALLBACK-DECISION.md` Option 1, this rolls to a future milestone rather than v1.0; active-listing ingestion (above) never depended on it and shipped independently. A future milestone could revisit via reapplication (e.g. as a registered business entity) or the paid PriceCharting API fallback (Option 2, requires an explicit scope conversation since it changes the "official eBay APIs only" constraint) — no decision made yet on which path, if any, to pursue next.
...
---
*Last updated: 2026-08-02 after v1.0 milestone completion — Phases 1-7 shipped and closed as the complete v1.0 product (active-listing price tracking only). The Marketplace Insights Growth Check was denied 2026-08-02; sold-price integration (formerly Phase 8) is deferred to a future milestone per `FALLBACK-DECISION.md` Option 1, not part of v1.0. The full pipeline was independently re-verified against real production data before close (109 ingestion runs, live API curl-checks) — see Context "v1.0 Shipped State".*
